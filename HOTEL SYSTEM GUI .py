...
# Create table
'''
c.execute("""CREATE TABLE roomdata (
t_r number,
r_r number,
t_s number,
rn number ,
...
Label(sroot, text="Made by -", font='Timesnewroman 20 ', bg='white', fg='black').place(x=535, y=90)
Label(sroot, text="Iande", font='Timesnewroman 20 ', bg='white', fg='grey').place(x=535, y=180)
Label(sroot, text="arahaman@gmail.com", font='Timesnewroman 10', bg='white', fg='grey').place(x=535, y=270)
...
def mainroot():
    ...
    path = "hotelpic/newestbg5.jpg"
    img = ImageTk.PhotoImage(Image.open(path))
    label = Label(top_frame, image=img, height=70, width=1080)
    label.image = img
    label.place(x=0, y=0)
    top_frame.place(x=0, y=0)
    tf_label = Label(top_frame, text='Grand Regency Hotel Management System', font='msserif 33', fg='blue', bg='gray89',
                     height=70)
    ...
    def hotel_status():
        ...
        path = "hotelpic/newbg6lf.jpg"
        img = ImageTk.PhotoImage(Image.open(path))
        label = Label(b_frame, image=img, height=400, width=1080)
        label.image = img
        label.place(x=0, y=0)
        c.execute("SELECT * FROM roomdata")
        x = c.fetchall()
        # print(x)
        c.execute("SELECT count(rn) FROM roomdata")
        x = c.fetchone()
        print(x)
        c.execute("SELECT count(rn)  FROM roomdata WHERE roomdata = 'Reserved'")
        y = c.fetchone()
        print(y)
        tor = x[0]
        rer = y[0]
        ...
        # -----------------------------------------------------------------
        nl = Label(b_frame, text='Made by Iande', fg='black', bg='gray91', font='msserif 8')
    ...
    def staff():
        ...
        Label(emp1inf, text="Mr. Iande Mw", bg='white', fg="Grey", font='msserif 10').place(x=60, y=37)
        Label(emp1inf, text="Extention : 025", bg='white', fg="Grey", font='msserif 10').place(x=60, y=59)
        Label(emp1inf, text="Mail : Manager@grandregency.com", bg='white', fg="Grey", font='msserif 10').place(x=60,
                                                                                                               y=83)
        emp1inf.pack_propagate(False)
        emp1inf.place(x=117, y=1)

        emp1f = Frame(b_frame)
        path2 = "hotelpic/receptionnew.jpg"
        ...
        emp1f = Frame(b_frame)
        path4 = "hotelpic/roomservicenew.jpg"
    ...
    def rooms():
        ...
        def roomdet(rno):
            ...
            c.execute("select * from roomdata where rn = ?", (rno,))
            roomdata = c.fetchall()
            # print (roomdata)
            ...
            Label(smf1, text=str(roomdata[0]) + str(roomdata[1]) , fg='cyan4', bg='white', font='msserif 35').pack()
    ...
    def payments():
        ...
        def getid(event=None):
            ...
            c.execute("SELECT * FROM payments where id = ?", (idd,))
            x = c.fetchall()
            # print(x)
            c.execute("SELECT day,month,year,time,totalamt,r_n FROM payments WHERE id = ?", (idd,))
            yy = c.fetchone()
            print(yy)
            # print (x)
            if (yy != None):
    ...
    def reserve():
        ...
        def booking():
            ...
            # cur.execute("create table if not exists paymentsf(id number  primary key,f_name varchar,l_name varchar,c_number varchar,email varchar , r_n number ,day varchar,month varchar,year varchar,time varchar , method varchar)")
                ...
                c.execute("SELECT roomdata FROM roomdata WHERE rn = ?", (roomn.get(),))
                temp = list.sort(c.fetchone())
                ...
                if temp[0] == 'Reserved':
                    ...
                    # global pmethod
                    c.execute("SELECT price FROM roomdata WHERE rn = (?)", (roomn.get(),))
                    rp = c.fetchone()
                    print(rp)
                    ...
                    def f():
                        if pmethod != '':
                            print(pmethod.get())
                            print('pmethod value')
                            c.execute("SELECT id from payments order by id desc")
                            x = c.fetchone()
                            cid = int(x[0])
                            cid += 1
                            # print (cid)
                            # print (pmethod.get())
                            # cur.execute("create table if not exists paymentsf(id number  primary key,f_name varchar,l_name varchar,c_number varchar,email varchar , r_n number ,day varchar,month varchar,year varchar,time varchar , method varchar,totalamt varchar)")
                            c.execute("INSERT INTO paymentsf VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (
                            ...
                            c.execute("UPDATE roomdata SET roomdata='Reserved' where rn = ? ", (roomn.get(),))
        ...
        def unreserve():
            if (roomn.get() == 'Enter Room Number') or (roomn.get() == ''):
                ...
                c.execute("UPDATE roomdata SET roomdata='Unreserved' where rn = ? ", (roomn.get(),))
        ...
        def findrooms():
            c.execute(
                'SELECT  rn,price,roomdata FROM roomdata WHERE beds = ? and ac = ? and tv = ? and internet = ? order by price asc',
